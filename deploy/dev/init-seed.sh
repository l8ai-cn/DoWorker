...
#   POSTGRES_USER     - PostgreSQL 用户 (默认: agentcloud)
#   POSTGRES_PASSWORD - PostgreSQL 密码 (默认: agentcloud_dev)
#   POSTGRES_DB       - PostgreSQL 数据库 (默认: agentcloud)
#
# =============================================================================
...
get_db_params() {
    ...
    DB_USER="${POSTGRES_USER:-agentcloud}"
    DB_PASSWORD="${POSTGRES_PASSWORD:-agentcloud_dev}"
    DB_NAME="${POSTGRES_DB:-agentcloud}"
...
check_migrations() {
    ...
    if ! psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -c "SELECT 1 FROM users LIMIT 1" &>/dev/null; then
        ...
        echo ""
        echo "运行迁移命令："
        echo "  ./dev.sh                     # 自动跑 migrate oneshot + 启动全栈"
        exit 1
    fi
    success "数据库迁移已完成"
}
...
apply_seed() {
    ...
    if psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -f "$SEED_FILE"; then
        ...
        echo ""
        echo "测试账号信息："
        echo "  Email:    dev@agentcloud.local"
        echo "  Password: AdminAb123456"
        echo "  Org:      dev-org"
        echo ""
        echo "Runner 信息："
        echo "  Node IDs:   dev-runner, dev-runner-2, dev-runner-claude, dev-runner-codex, dev-runner-video-studio, dev-runner-cursor, dev-runner-gemini, dev-runner-loopal, dev-runner-do-agent, dev-runner-grok, dev-runner-openclaw, dev-runner-hermes, dev-runner-aider, dev-runner-opencode, admin-workspace-do-agent"
        echo "  Auth Token: dev-runner-token"
    else
        error "Seed 数据应用失败"
        exit 1
    fi
...
check_status() {
    ...
    psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -c \
        "SELECT id, email, username, is_active FROM users WHERE email = 'dev@agentcloud.local'" \
        --tuples-only 2>/dev/null | while read line; do
        if [[ -n "$line" ]]; then
            echo "  ✓ dev@agentcloud.local 已存在"
        fi
    done
...
DELETE FROM users WHERE email = 'dev@agentcloud.local';
EOF
...
show_help() {
    ...
    echo "  POSTGRES_USER     PostgreSQL 用户 (默认: agentcloud)"
    echo "  POSTGRES_PASSWORD PostgreSQL 密码 (默认: agentcloud_dev)"
    echo "  POSTGRES_DB       PostgreSQL 数据库 (默认: agentcloud)"
}
