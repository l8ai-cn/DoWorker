...
API="${HIVE_API_URL:-http://localhost:10015}"
...
preflight_check_migrations() {
  ...
  if [[ -n "$rows" && "$rows" != "1|f" ]]; then
    echo "hive smoke: schema_migrations is inconsistent (count|dirty = ${rows})"
...
run_suite "S1 session wire + elicitation + terminal" "hive-s1-smoke.mjs"
