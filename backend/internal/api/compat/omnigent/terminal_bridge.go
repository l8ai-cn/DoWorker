...
type omnigentResize struct {
	Type string `json:"type"`
	Cols int    `json:"cols"`
	Rows int    `json:"rows"`
}

func parseOmnigentResize(text string) (cols, rows uint16, ok bool) {
	var r omnigentResize
...
func bridgeTerminalWS(client, upstream *websocket.Conn, readOnly bool) {
	...
	for {
		...
		case websocket.TextMessage:
			if cols, rows, ok := parseOmnigentResize(string(data)); ok {
