...
func lastTaskErrorFromPod(pod *podDomain.Pod) map[string]any {
	if pod == nil || pod.ErrorMessage == nil {
		return nil
	}
	msg := strings.TrimSpace(*pod.ErrorMessage)
	if msg == "" {
		return nil
	}
	return map[string]any{"code": "agent_failed", "message": msg}
}

func mergeSessionGet(item conversationListItem, wire sessionWire, pod *podDomain.Pod) map[string]any {
	out := map[string]any{
