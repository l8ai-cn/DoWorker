...
type PodReconcileService struct {
	pc *PodCoordinator
}

func newPodReconcileService(pc *PodCoordinator) *PodReconcileService {
	return &PodReconcileService{pc: pc}
}

func (r *PodReconcileService) reconcile(ctx context.Context, runnerID int64, reportedPods map[string]bool) {
	pc := r.pc
	now := time.Now()
	...
	for podKey := range reportedPods {
		...
		if pod.Status == agentpod.StatusInitializing {
			...
			if count >= initRecoverThreshold {
				...
				r.recoverPodStatus(ctx, podKey, runnerID, agentpod.StatusInitializing, now)
			}
		}
	}

	r.reconcileMissingPods(ctx, runnerID, reportedPods, now)
	r.syncPodCount(ctx, runnerID, reportedPods)
}

func (r *PodReconcileService) recoverPodStatus(ctx context.Context, podKey string, runnerID int64, fromStatus string, now time.Time) {
	pc := r.pc
	...
	if rowsAffected > 0 {
		...
		pc.notifyStatusChange(podKey, agentpod.StatusRunning, "")
	}
}

func (r *PodReconcileService) reconcileMissingPods(ctx context.Context, runnerID int64, reportedPods map[string]bool, now time.Time) {
	pc := r.pc
