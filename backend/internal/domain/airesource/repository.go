...
type Repository interface {
	...
	SaveConnectionMetadata(ctx context.Context, connection *Connection) error
	DeleteConnection(ctx context.Context, id, expectedRevision int64, expectedUpdatedAt time.Time) error
	...
	SaveResourceMetadata(ctx context.Context, resource *ModelResource) error
	DeleteResource(ctx context.Context, id, expectedRevision int64, expectedUpdatedAt time.Time) error
	...
	SetValidationState(ctx context.Context, connectionID, expectedRevision int64, expectedCredentialsEncrypted string, status ConnectionStatus, at time.Time, validationError string) (int64, error)
}
