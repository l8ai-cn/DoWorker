...
func TestAIResourceRepositorySetsValidationStateAtomically(t *testing.T) {
	...
	revision, err := repo.SetValidationState(context.Background(), connection.ID, connection.Revision, airesource.ConnectionStatusInvalid, at, "credentials rejected")
	require.NoError(t, err)
	assert.Equal(t, connection.Revision+1, revision)
	...
	_, err = repo.SetValidationState(context.Background(), connection.ID, revision, airesource.ConnectionStatusValid, at.Add(time.Hour), "")
	require.Error(t, err)
	storedConnection, err = repo.GetConnectionByID(context.Background(), connection.ID)
	require.NoError(t, err)
	storedResource, err = repo.GetResourceByID(context.Background(), resource.ID)
