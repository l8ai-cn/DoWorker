...
func (m *MockStorage) DownloadRange(ctx context.Context, key string, start int64, end int64) (io.ReadCloser, int64, error) {
	if m.DownloadErr != nil {
		return nil, 0, m.DownloadErr
	}
	m.mu.RLock()
	...
	if start < 0 || end < start || end >= int64(len(f.data)) {
		return nil, 0, fmt.Errorf("mock storage: invalid range")
	}
...
func (m *MockStorage) PutFileData(key string, data []byte, contentType string) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.files[key] = &mockFile{
		key: key, data: append([]byte(nil), data...),
