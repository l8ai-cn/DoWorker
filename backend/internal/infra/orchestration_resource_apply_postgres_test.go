...
func TestOrchestrationIdempotentApplyReturnsExistingResultOnReplay(t *testing.T) {
	_, repo := orchestrationPostgresRepository(t)
	plan := orchestrationApplyTestCreatePlan(t)
	require.NoError(t, repo.CreatePlan(context.Background(), plan))
	builder := func(state orchestrationservice.LockedApplyState) (
	...
	first, err := repo.runIdempotentApplyTransaction(
		context.Background(),
		plan.Scope,
		plan.ID,
		plan.Target.Kind,
	...
	replayed, err := repo.runIdempotentApplyTransaction(
		context.Background(),
		plan.Scope,
		plan.ID,
		plan.Target.Kind,
