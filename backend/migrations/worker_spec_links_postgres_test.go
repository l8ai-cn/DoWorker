...
func TestMigration000199WorkerSpecLinksPostgres(t *testing.T) {
	...
	schema := fmt.Sprintf("worker_spec_links_%d", time.Now().UnixNano())
...
`))
	...
	require.True(t, postgresColumnExists(ctx, t, conn, "pods", "worker_spec_snapshot_id"))
	require.NoError(t, execSQL(ctx, conn, `
INSERT INTO pods(id, organization_id, worker_spec_snapshot_id) VALUES (1, 77, 10)
`))
	err = execSQL(ctx, conn, `
INSERT INTO pods(id, organization_id, worker_spec_snapshot_id) VALUES (2, 78, 10)
`)
	require.Error(t, err)
	...
	require.False(t, postgresColumnExists(ctx, t, conn, "pods", "worker_spec_snapshot_id"))
}
