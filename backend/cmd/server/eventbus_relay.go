...
func setupRelayTokenRefreshCallback(
	...
	runnerConnMgr.SetRequestRelayTokenCallback(func(runnerID int64, data *runnerv1.RequestRelayTokenEvent) {
		go refreshRunnerRelayToken(db, tokenGenerator, commandSender, runnerID, data)
	})
}

func refreshRunnerRelayToken(
	db *gorm.DB,
	tokenGenerator *relay.TokenGenerator,
	commandSender runner.RunnerCommandSender,
	runnerID int64,
	data *runnerv1.RequestRelayTokenEvent,
) {
	...
	if err := db.WithContext(ctx).Table("pods").Where("pod_key = ?", data.PodKey).First(&pod).Error; err != nil {
