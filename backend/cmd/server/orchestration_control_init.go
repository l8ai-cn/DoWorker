...
func initializeOrchestrationControl(
...
) (*controlservice.Service, error) {
	if db == nil || organizations == nil || workerCreation == nil {
		return nil, fmt.Errorf("orchestration control dependencies are incomplete")
	}
	registry := resource.NewRegistry()
	if err := resource.RegisterWorkerSchemas(registry); err != nil {
		return nil, fmt.Errorf("register orchestration schemas: %w", err)
	}
	if err := resource.RegisterDefinitionSchemas(registry); err != nil {
		return nil, fmt.Errorf("register orchestration definition schemas: %w", err)
	}
	...
	return controlservice.NewService(controlservice.ServiceDeps{
