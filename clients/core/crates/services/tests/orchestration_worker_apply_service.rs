...
#[tokio::test]
async fn worker_create_service_decodes_and_encodes_typed_messages() {
    ...
    assert!(service
        .create_worker_from_plan_connect(&[0xff])
        .await
        .is_err());
    ...
    let bytes = service
        .create_worker_from_plan_connect(&request.encode_to_vec())
        .await
        .expect("create worker");
    let applied = resource::CreateWorkerFromPlanResponse::decode(&*bytes).expect("decode response");
