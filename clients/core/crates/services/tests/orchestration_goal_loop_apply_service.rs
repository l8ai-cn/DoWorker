...
#[tokio::test]
async fn goal_loop_apply_service_decodes_and_encodes_typed_messages() {
    ...
    assert!(service
        .create_goal_loop_from_plan_connect(&[0xff])
        .await
        .is_err());
    let request = resource::CreateGoalLoopFromPlanRequest {
        org_slug: "acme".into(),
        plan_id: "11111111-1111-4111-8111-111111111111".into(),
    };
    let response = resource::CreateGoalLoopFromPlanResponse {
        resource: None,
        goal_loop_id: 83,
        worker_spec_snapshot_id: 93,
        resource_revision: 5,
    };
    ...
    let bytes = service
        .create_goal_loop_from_plan_connect(&request.encode_to_vec())
        .await
        .expect("create goal loop");
    let applied =
        resource::CreateGoalLoopFromPlanResponse::decode(&*bytes).expect("decode response");

    assert_eq!(applied.goal_loop_id, 83);
    assert_eq!(applied.worker_spec_snapshot_id, 93);
    assert_eq!(applied.resource_revision, 5);
}
