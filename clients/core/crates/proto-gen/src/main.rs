...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for domain in topological_order(DOMAINS)? {
        ...
        println!("generated {} → crates/proto/{}", domain.name, domain.name);
    }
    Ok(())
}
...
fn generate_domain(
    ...
    for dep in domain.deps {
        // Use `r_loop` (not `r#loop`) in extern_path — prost-build mangles raw
        // identifiers. The loop_proto crate exposes `proto::r_loop` as an alias.
        let dep_mod = if *dep == "loop" { "r_loop".into() } else { rust_mod(dep) };
        let path = format!("::{dep}_proto::proto::{dep_mod}::v1");
        config.extern_path(format!(".proto.{dep}.v1"), path);
    }

    let protos: Vec<PathBuf> = domain
        .srcs
        .iter()
        .map(|s| proto_root.join(domain.name).join("v1").join(s))
        .collect();
    for p in &protos {
        if !p.is_file() {
...
fn write_lib_rs(
    ...
    // prost escapes the `loop` keyword as `r#loop` in the output filename.
    let candidates = [
        format!("proto.{}.v1.rs", domain.name),
        format!("proto.{}.v1.rs", rust_mod(domain.name)),
    ];
    ...
    // prost-build turns `r#loop` in extern_path into `r_loop` in generated code.
    let body = fs::read_to_string(&generated)?.replace("::r_loop::", "::r#loop::");
    let mod_name = rust_mod(domain.name);
    let alias = if domain.name == "loop" {
        // Dependents may still emit `proto::r_loop::v1` if a path slips through.
