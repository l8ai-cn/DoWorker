...
pub(crate) fn loop_from_proto(p: ProtoLoop) -> LoopData {
    LoopData {
        id: p.id,
        slug: p.slug,
        name: p.name,
...
pub(crate) fn run_from_proto(p: ProtoLoopRun) -> LoopRunData {
    LoopRunData {
        id: p.id,
...
pub(crate) fn loop_to_proto(l: &LoopData) -> ProtoLoop {
    ProtoLoop {
        id: l.id,
        slug: l.slug.clone(),
        name: l.name.clone(),
...
pub(crate) fn run_to_proto(r: &LoopRunData) -> ProtoLoopRun {
    ProtoLoopRun {
        id: r.id,
