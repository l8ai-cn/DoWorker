...
function policy(overrides: Partial<Record<string, unknown>> = {}) {
  return {
    ...
    name: "block_canada",
    type: "python",
    handler: "do-worker.policies.block_canada",
    factory_params: null,
    enabled: true,
    created_at: 1,
    updated_at: null,
  ...
  it("renders each policy with its handler, a Disabled badge, and parameters", async () => {
    setPolicies([
      ...
      policy({
        id: "p2",
        name: "rate_limit",
        handler: "do-worker.policies.rate_limit",
    ...
    expect(screen.getByText("do-worker.policies.block_canada")).toBeInTheDocument();
...
describe("PoliciesPage actions", () => {
  ...
  it("adds a global policy from the registry via the Add dialog", async () => {
    ...
    expect(addMutate).toHaveBeenCalledWith(
      { name: "block_canada", type: "python", handler: "do-worker.policies.block_canada" },
      expect.anything(),
    );
  });
});
