...
function renderArtifact(fileId: string, filename: string | null, contentType: string | null = null) {
  return render(
...
describe("OutputFileArtifact", () => {
  ...
  it("renders an MP4 MIME response as video without relying on its filename", () => {
    renderArtifact("file-video", "provider-output", "video/mp4");

    expect(screen.getByLabelText("provider-output").tagName).toBe("VIDEO");
  });
