...
export type ExpertDispatchResult =
  ...
  | { kind: "pod_only"; podKey: string; message: string };
...
export async function dispatchLiveExpertTask(
  ...
  if (expert.interaction_mode === "acp") {
    const session = await createSession(expert.agent_slug, title, text);
  ...
  return {
    ...
    message:
      warning ??
      `Pod ${pod.pod_key} 已创建（PTY 模式）。会话链接就绪后可在首页看到。`,
  };
}
...
export async function dispatchExpertBySlug(
  ...
  return dispatchLiveExpertTask(expert, prompt);
}
