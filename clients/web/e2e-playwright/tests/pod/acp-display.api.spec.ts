...
test.describe("ACP Pod API", () => {
   ...
   */
  test("create ACP pod with WorkerSpecDraft", async ({ api }) => {
    const cc = await api.connect();
    const resp = await createE2EEchoPod(cc, { mode: "acp" }) as { pod: Pod };
  ...
  test("send prompt to pod via API", async ({ api }) => {
    const cc = await api.connect();
    const created = await createE2EEchoPod(cc, { mode: "acp" }) as { pod: Pod };
