// WorkflowData (web snake_case) ↔ proto.workflow.v1.Workflow (camelCase) mapping.
// `fromProtoWorkflow` lives in connect/workflowConnect.ts (used by Connect-RPC
// adapters). This module adds `workflowToProtoWorkflow` + `workflowRunToProtoWorkflowRun`,
...
import {
  WorkflowSchema, WorkflowRunSchema,
  type Workflow as ProtoWorkflow, type WorkflowRun as ProtoWorkflowRun,
} from "@proto/workflow/v1/workflow_pb";
import type { WorkflowData, WorkflowRunData } from "@/lib/viewModels/workflow";
...
export function workflowToProtoWorkflow(l: WorkflowData): ProtoWorkflow {
  return protoCreate(WorkflowSchema, {
...
export function workflowRunToProtoWorkflowRun(r: WorkflowRunData): ProtoWorkflowRun {
  return protoCreate(WorkflowRunSchema, {
    id: BigInt(r.id),
    workflowId: BigInt(r.workflow_id),
