...
export default function OnboardingPage() {
  ...
  const title = user?.name
    ? t("auth.onboarding.welcomeWithName", { name: user.name })
    : t("auth.onboarding.welcome");

  return (
    <AuthShell title={title} subtitle={t("auth.onboarding.setupWorkspace")}>
      {error && (
      ...
      {showInviteInput ? (
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <Button className="flex-1" onClick={handleJoinWithInvite} disabled={loading || !inviteCode.trim()}>
