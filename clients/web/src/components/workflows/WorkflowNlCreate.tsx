  ...
  async function submit() {
    ...
    try {
      const result = await quickTaskApi.create({
        prompt: buildWorkflowAiGuidePrompt(trimmed),
        alias: t("workflows.aiGuidePodAlias"),
      });
      toast.success(t("workflows.aiGuideStarted"), {
        description: t("workflows.aiGuideStartedDesc"),
      });
      router.push(`/${currentOrg.slug}/workspace?pod=${encodeURIComponent(result.pod_key)}`);
    } catch (err) {
      const code = getApiErrorCode(err);
      if (code && WIZARD_FALLBACK_CODES.has(code)) {
        toast.info(t("workflows.aiGuideFallback"));
        onNeedsWizard(trimmed);
      } else {
        toast.error(getLocalizedErrorMessage(err, t, t("workflows.aiGuideError")));
      }
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <section className="rounded-xl bg-surface-raised p-5 ring-1 ring-border/35">
      <div className="mb-3 flex items-center gap-2">
        ...
        <h2 className="text-sm font-medium">{t("workflows.aiGuideTitle")}</h2>
      </div>
      <p className="mb-3 text-xs text-muted-foreground">{t("workflows.aiGuideDescription")}</p>
      <Textarea
        value={idea}
        onChange={(e) => setIdea(e.target.value)}
        placeholder={t("workflows.aiGuidePlaceholder")}
        rows={3}
        maxLength={4000}
        disabled={submitting}
        onKeyDown={(e) => {
      ...
      <div className="mt-2 flex flex-wrap gap-1.5">
        {EXAMPLE_KEYS.map((key) => (
          <button
            ...
            type="button"
            disabled={submitting}
            onClick={() => setIdea(t(`workflows.${key}`))}
