...
import { WorkflowRunData } from "@/stores/workflow";
...
import { STATUS_CONFIG } from "./workflow-run-status";
    ...
    <div>
      ...
      >
        ...
        <span className="hidden sm:inline-flex items-center gap-1 text-xs text-muted-foreground w-16 flex-shrink-0">
          ...
          <span>{run.trigger_type === "cron" ? t("workflows.triggerTypeCron") : run.trigger_type === "api" ? t("workflows.triggerTypeApi") : t("workflows.triggerTypeManual")}</span>
        </span>
        ...
        <div className="flex items-center gap-1 flex-shrink-0 justify-end" onClick={(e) => e.stopPropagation()}>
          {run.pod_key && onViewTerminal && (
            <Button size="sm" variant="ghost" className="h-7 text-xs gap-1" onClick={(e) => { e.stopPropagation(); onViewTerminal(run.pod_key!); }}>
              ...
              {run.status === "running" || run.status === "pending" ? t("workflows.viewTerminal") : t("workflows.viewLogs")}
            </Button>
          )}
          {isRunning && onCancel && (
      ...
      {expanded && hasDetails && (
        <div className="ml-6 mr-3 mb-1 p-3 rounded-lg surface-card bg-muted/30 space-y-2 text-xs">
          {run.error_message && (
            <div><span className="font-medium text-danger">{t("workflows.errorMessage")}:</span><span className="ml-1.5 text-foreground/80">{run.error_message}</span></div>
          )}
          {run.exit_summary && (
            <div><span className="font-medium text-muted-foreground">{t("workflows.exitSummary")}:</span><span className="ml-1.5 text-foreground/80">{run.exit_summary}</span></div>
          )}
          {run.resolved_prompt && (
            <div>
              <span className="font-medium text-muted-foreground">{t("workflows.resolvedPrompt")}:</span>
