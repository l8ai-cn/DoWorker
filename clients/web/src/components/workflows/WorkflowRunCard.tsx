...
import type { WorkflowRunData, RunStatus } from "@/stores/workflow";
...
interface WorkflowRunCardProps {
  ...
  onOpen?: (run: WorkflowRunData) => void;
...
function triggerLabel(t: WorkflowRunCardProps["t"], type: string) {
  switch (type) {
    case "cron":
      return t("workflows.triggerTypeCron");
    case "api":
      return t("workflows.triggerTypeApi");
    default:
      return t("workflows.triggerTypeManual");
  }
}

export function WorkflowRunCard({ run, t, onOpen, onCancel }: WorkflowRunCardProps) {
  const tone = RUN_TONE[run.status] ?? RUN_TONE.completed;
  const statusKey = `workflows.status${run.status.charAt(0).toUpperCase()}${run.status.slice(1)}`;
    ...
    >
      ...
      <div className="hidden min-w-0 shrink-0 flex-col gap-0.5 lg:flex lg:w-[220px]">
        <span className="truncate text-xs text-foreground">
          {run.exit_summary || run.error_message || t("workflows.noArtifact")}
        </span>
        {run.resolved_prompt && (
          <span className="truncate font-mono text-[11px] text-muted-foreground">
      ...
      <div className="flex shrink-0 items-center gap-2">
        ...
        {onOpen && (
          ...
          >
            {run.pod_key ? t("workflows.openRun") : t("workflows.viewLogs")} →
          </button>
        )}
      </div>
    </div>
