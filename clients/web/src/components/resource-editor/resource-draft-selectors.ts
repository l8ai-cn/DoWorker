...
import { isPendingResourcePlan } from "./resource-plan-status";

export function resourceDraftCanSubmit(state: ResourceDraftState): boolean {
  if (state.source.error || (state.source.dirty && state.mode !== "yaml")) {
    return false;
  }
  return resourceDraftCanSubmitDraft(state.draft);
}
