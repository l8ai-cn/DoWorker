...
同一清单也可以作为 Runner MCP `create_pod` 或 `create_workflow` 的 `resource`
参数提交。MCP 接受 JSON 对象，不接受 YAML 字符串；Backend 仍执行相同的
Validate、Plan、权限复核和 typed Apply。Agent 不能通过 MCP 直接传模型密钥、
Runner ID、AgentFile 或 Prompt 文本来绕过资源引用。
