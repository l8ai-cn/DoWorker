...
Do Worker 的 Runner 是用户自托管的守护进程，运行在用户的机器上执行 AI Agent 任务。当前 Runner 与 Backend 通过 WebSocket + JSON 通信，使用 `node_id + auth_token` 认证。
...
**关键设计**：Runner 只信任 Do Worker 私有 CA，不信任系统 CA。攻击者即使获得 Let's Encrypt 等公共 CA 签发的证书，也无法伪造服务端。
...
**对 Do Worker 的影响**：Runner 使用长连接（gRPC stream），握手只发生一次，总体 CPU 增加 < 1%。
...
# 伪造证书应被拒绝（使用非 Do Worker CA 签发的证书）
