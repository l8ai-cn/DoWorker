...
2. [x] `hive_smoke.sh` 增 parity suite（`hive-s5-smoke.mjs`，shell 标签「S5 full parity」；与 hive-execution-plan 的 S5 审查修复不同名域）
3. [x] `tests/hive-smoke/BUILD.bazel` `hive_smoke_scripts` 纳入 `hive-s5-smoke.mjs`（Bazel 路径与 shell 一致）
4. [x] 更新本文档状态头
5. [ ] `bash deploy/dev/hive_smoke.sh` S0–S5 全绿（需 dev stack + migration 175）
6. [ ] web-user 全页面手工验收（console 零 404/501）
7. [x] 更新 `hive-execution-plan.md` Omnigent parity 节

验收：`bash deploy/dev/hive_smoke.sh` 全绿；web-user 手工走查通过。
