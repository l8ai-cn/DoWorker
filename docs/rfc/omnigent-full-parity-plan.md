...
> 状态：W1–W8 已落地 | 标准：web-user 每一个 `/v1/*` 调用都有真实实现（非 stub）
> 前置：`agent-platform-execution-plan.md` S0–S7 已完成（compat MVP + 机制层）
...
验收：FileViewer 读/写文件、ChangedFiles 面板、diff 视图、workspace 搜索全通；`session-s5-smoke.mjs` 断言 write→read 回读一致 + git diff 出现修改。
...
1. session 级 policy：`permission_policies.session_id` 列（migration 175）；create/delete 后即时 `SendUpdatePodPolicyRules` 推送该 Pod
...
1. [x] `GET /v1/agents?after=` 游标分页
2. [x] `session_compat_smoke.sh` 增 parity suite（`session-s5-smoke.mjs`，shell 标签「S5 full parity」）
3. [x] Session compatibility smoke 纳入 `session-s5-smoke.mjs`
4. [x] 更新本文档状态头
5. [ ] `bash deploy/dev/session_compat_smoke.sh` S0–S5 全绿（需 dev stack + migration 175）
6. [ ] web-user 全页面手工验收（console 零 404/501）
7. [x] 更新 `agent-platform-execution-plan.md` Omnigent parity 节

验收：`bash deploy/dev/session_compat_smoke.sh` 全绿；web-user 手工走查通过。
...
每个工作包完成即本地 commit（不开 PR），跑 `bash deploy/dev/session_compat_smoke.sh` 回归。
