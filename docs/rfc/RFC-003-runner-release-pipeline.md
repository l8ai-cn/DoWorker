...
Agent Cloud Runner 需要支持多平台分发，包括：
...
- **源码仓库**: GitLab `aio/agentcloud` (monorepo)
- **发布仓库**: GitHub `l8ai-cn/AgentCloud`
...
```yaml
version: 2
project_name: agentcloud-runner
...
nfpms:
  - id: runner-deb-rpm
    package_name: agentcloud-runner
    formats: [deb, rpm, apk]
```
...
package:macos-dmg:
  ...
  script:
    ...
    - export VERSION="${CI_COMMIT_TAG:-dev}"
    # 创建 .app bundle
    - mkdir -p "Agent Cloud Runner.app/Contents/MacOS"
    - cp runner-desktop-darwin-universal "Agent Cloud Runner.app/Contents/MacOS/runner-desktop"
    # 创建 DMG
    - DMG_NAME="Agent Cloud-Runner-${VERSION}-darwin-universal.dmg"
    - create-dmg --volname "Agent Cloud Runner" "$DMG_NAME" "Agent Cloud Runner.app" || true
...
# 3. 等待 CI Pipeline 完成
# 4. 检查 GitHub Release: https://github.com/l8ai-cn/AgentCloud/releases
```
...
| 文件 | 说明 |
|------|------|
| `agentcloud-runner_X.Y.Z_darwin_all.tar.gz` | macOS CLI (Universal) |
| `agentcloud-runner_X.Y.Z_darwin_amd64.tar.gz` | macOS CLI (Intel) |
| `agentcloud-runner_X.Y.Z_darwin_arm64.tar.gz` | macOS CLI (Apple Silicon) |
| `agentcloud-runner_X.Y.Z_linux_amd64.tar.gz` | Linux CLI (x64) |
| `agentcloud-runner_X.Y.Z_linux_arm64.tar.gz` | Linux CLI (ARM64) |
| `agentcloud-runner_X.Y.Z_linux_amd64.deb` | Debian/Ubuntu 包 |
| `agentcloud-runner_X.Y.Z_linux_amd64.rpm` | RHEL/CentOS/Fedora 包 |
| `agentcloud-runner_X.Y.Z_linux_amd64.apk` | Alpine 包 |
| `agentcloud-runner_X.Y.Z_windows_amd64.zip` | Windows CLI (x64) |
| `agentcloud-runner_X.Y.Z_windows_arm64.zip` | Windows CLI (ARM64) |
| `Agent Cloud-Runner-vX.Y.Z-darwin-universal.dmg` | macOS Desktop (Universal) |
