...
| **P3** | B/C/D 完整 + E | policy 热更新；实时 usage USD；resume/fork；CreatePod 简易模式 | 阶段 2–3 平台机制整合验收 |
...
> 最后补平台机制（S3）。S0 不通过，S1–S3 全部无法验收。
...
- [x] S1.1 浏览器 E2E：登录 → 选 agent → 建 session → 发消息 → 气泡渲染
      （`node tests/session-compat-smoke/session-browser-integration.mjs` 全绿，2026-07-05）
...
- [x] S1.3 elicitation 闭环：`scenario=permission_request_edit` →
      `POST .../elicitations/{id}/resolve` → assistant 继续（`session-s1-smoke.mjs`）
- [x] S1.4 Terminal attach WS：`pty_only` session → relay 字节（`session-s1-smoke.mjs`）

验收：无 console error；一次完整对话 + 一次审批 + 一次终端 attach。

### S2 — compat API 缺口（按 web-user 404 影响排序）

- [x] S2.1 `GET /v1/harnesses`：builtin agents → `{id, label}`（`session-s2-smoke.mjs`）
...
验收：`bash deploy/dev/session_compat_smoke.sh` 全绿（2026-07-05）。

### S4 — 收尾

- [x] S4.1 统一 smoke：`deploy/dev/session_compat_smoke.sh` 串联 S0–S4。
- [x] S4.2 CI：`bazel.yml` smoke job 跑 `runner_runtime_contract_test`
- [x] S4.3 `reset_runners` 改为 docker cp 热更新，跳过 apt image rebuild
- [x] S4.4 更新本文件与 `agent-platform-integration-plan.md` 状态头

### S5 — 审查修复（2026-07-05 复查后补强）

- [x] T1 `schema_migrations` 修复：删除手工插入的 164 双行；migrate oneshot
      up/idempotence 验证通过（version=166, dirty=f）；`session_compat_smoke.sh`
      preflight 加 `count|dirty = 1|f` 一致性检查
- [x] T2 policy 热推送行为断言：`session-s3-smoke.mjs` 对在跑 Pod 推
...
- [x] T5 CI dev-stack job：运行会话兼容 E2E（backend_only
      → pnpm + chromium → `bash deploy/dev/session_compat_smoke.sh` S0–S4 全量）

### S6 — P4 平台延伸（2026-07-05 纳入验收）

- [x] S6.1 read-state 落库：`session_read_states` 表 + GORM upsert；
      `PUT/GET` wire 经 Postgres 持久化（`session-s4-smoke.mjs`）
...
验收：`bash deploy/dev/session_compat_smoke.sh` S0–S4 全绿；migration `170`。
...
backend/migrations/000163_agent_platform_foundations.up.sql
clients/web/src/lib/agent-capability-axes.ts
clients/web-user/vite.config.ts
docs/rfc/agent-platform-execution-plan.md
```
