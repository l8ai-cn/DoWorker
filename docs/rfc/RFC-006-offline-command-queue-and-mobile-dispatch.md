...
#### A. POST `/api/v1/organizations/:slug/quick-tasks`

移动端极简下发入口（薄封装，内部全走 `PodOrchestrator`）。

请求：

```json
{
  "prompt":        "把登录页的 bug 修了",          // 必填，1..10000 字符
  "agent_slug":    "codex-cli",                    // 可选；空 = org 内按 slug 排序取首个可用
  "runner_id":     0,                              // 可选；0 = 亲和性自动选
  "repository_id": null,                           // 可选
  "alias":         "修登录 bug",                    // 可选，≤100 字符
  "queue_ttl_minutes": 30                          // 可选，1..1440，默认 30
}
```

内部映射：`prompt` → AgentfileLayer `PROMPT "..."`（复用 MCP 工具已有的转义函数逻辑）；`QueueIfUnavailable=true` 恒定；Cols/Rows 固定 120×40。

**Runner 选择与排队的交互**：`runner_id=0` 且亲和性选择返回 `ErrNoRunnerForAgent`（全部离线）时，回退策略为「选择该 org 该 agent **最近在线**的 Runner」（`runnerRepo` 按 `last_seen_at` 降序）入队；org 内该 agent 从未有过任何 Runner → 422。

响应：

```json
// 202 Accepted（两种子状态）
{ "pod_key": "pd-x7k2m9", "status": "initializing" }                       // 在线直发
{ "pod_key": "pd-x7k2m9", "status": "queued", "queue_position": 3,
  "expires_at": "2026-07-05T13:05:00Z" }                                   // 已排队
```
...
| HTTP | code | 场景 |
|---|---|---|
| 400 | `VALIDATION_ERROR` | prompt 为空/超长、ttl 越界 |
| 404 | `AGENT_NOT_FOUND` | agent_slug 无效 |
| 422 | `NO_RUNNER_FOR_AGENT` | org 内无任何（含历史）可承载 Runner |
| 429 | `QUEUE_FULL` | 目标 Runner 积压 ≥ max_queued_per_runner |
| 500 | `CONFIG_BUILD_FAILED` | AgentFile eval 失败 |
...
| 环境变量 | 默认 | 说明 |
|---|---|---|
| `PENDING_QUEUE_ENABLED` | `true` | 总开关；false 时 `queue_if_offline` 请求直接走现状失败路径 |
...
`backend/internal/api/rest/v1/quick_task_test.go`（httptest + gin，mock orchestrator，对齐 `pod_commands_test.go` 既有模式）

| 用例 | 断言 |
|---|---|
| `TestQuickTask_OnlineRunner_202Initializing` | 202、body 含 pod_key、status=initializing |
| `TestQuickTask_OfflineRunner_202Queued` | 202、status=queued、queue_position>0、expires_at 存在 |
| `TestQuickTask_EmptyPrompt_400` / `TestQuickTask_PromptTooLong_400` | 400 VALIDATION_ERROR |
| `TestQuickTask_UnknownAgent_404` | 404 AGENT_NOT_FOUND |
| `TestQuickTask_NoRunnerEver_422` | 422 NO_RUNNER_FOR_AGENT |
| `TestQuickTask_QueueFull_429` | 429 QUEUE_FULL |
| `TestQuickTask_PromptEscaping` | prompt 含 `"` / 换行 / `\` → AgentfileLayer 转义正确（防注入 AgentFile 声明） |
...
| 行为兼容 | 所有排队语义 opt-in；`PENDING_QUEUE_ENABLED=false` 一键回退 |
