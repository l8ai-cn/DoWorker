# LoomScript 语言与 AST 详细设计

- 日期：2026-07-14
- 状态：待评审
- 依赖：`2026-07-14-loom-bidirectional-feasibility-review.md`

## 1. 语言定位

LoomScript 是声明 Agent 执行过程的受限 DSL。它不是通用编程语言，不允许任意函数、动态加载、反射、文件读写或网络访问。所有外部行为必须通过注册的 Worker、Verifier、Skill、MCP Tool 或平台 Action 节点表达。
...
首期后端使用 Go 实现权威 parser、AST builder、type checker 和 formatter，结构沿用现有 `agentfile/lexer`、`agentfile/parser` 的分层方式，但 LoomScript 使用独立 package 和 token 集。
