# Loom 与 AgentsMesh 运行领域接入详细设计

- 日期：2026-07-14
- 状态：待评审
- 依赖：Loom Controller 可靠性与验证设计
...
AgentFile 继续是 WorkerSpec 的运行配置编译产物。Loom 不把控制流写入 AgentFile，也不在运行时重新解析最新 Skill/MCP 内容。
...
Rust Core 的 `LoomState` 接收 LoopRun/StepRun 事件并保存统一 projection。界面通过 ProgramVersion source map 将 opaque node id 映射到：
...
2. Phase 1：双向编辑和 Rust Core LoomState。
