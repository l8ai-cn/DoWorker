...
- 日期：2026-07-14
- 状态：待评审
- 依赖：`2026-07-14-loom-language-and-ast-design.md`
...
自定义积木是不可变、版本化的 AST 宏，不是 JavaScript 插件。定义源使用独立 LoomBlock DSL；LoopProgram 只允许 `use block` 和类型化调用。
...
1. 任一内置积木都能生成规范 LoomScript，并从该代码恢复同构 AST。
