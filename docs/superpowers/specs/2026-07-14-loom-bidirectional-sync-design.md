...
- 日期：2026-07-14
- 状态：待评审
- 依赖：LoomScript 语言与 AST、积木目录设计

## 1. 同步架构

```mermaid
flowchart LR
  Code["CodeMirror / LoomScript"] --> Tx["Draft Transaction"]
  Blocks["Blockly Workspace"] --> Tx
  Tx --> Core["Rust Core LoomDraftState"]
...
正式实现新增 Rust Core `LoomState` 并挂入共享 `AppState`。状态、服务和事件通过同一个 `AppRuntime` 暴露给 WASM；禁止在 React local state 或 Zustand 建立第二份业务草稿。
