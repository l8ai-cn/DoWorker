# Do Worker 移动端接入 Plan B
**状态：** 详细设计
**日期：** 2026-07-10
**本轮范围：** Plan B V1 自有移动入口实现、测试与提交前 blocker 修复。
## 1. 结论
采用 Plan B：复用 Do Worker 的 Pod、Relay、Runner、Wasm Core 与现有 Web
...
- `codexapp` 是独立工作台，不认识 Do Worker 的 Org、Pod、Runner 与 RBAC。

Do Worker 已具备上述底座能力。直接引入 lulu 会形成双运行时、双 Relay、
