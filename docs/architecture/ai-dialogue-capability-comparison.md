...
> 对比对象：Agent Cloud、Codex CLI、Claude Code、Gemini CLI、OpenCode。
> 证据日期：2026-07-10。外部结论仅使用各项目官方文档或官方源码。

## 结论摘要

五个项目的产品边界不同。Codex CLI、Claude Code、Gemini CLI 和
OpenCode 以本地 Agent 会话为中心，直接控制本机工具运行时；Agent Cloud
是多租户控制平面，必须在浏览器、Relay、Runner、Agent 协议、资源凭据和
组织授权之间仍保持连贯对话体验。

Agent Cloud 的核心优势是异构 Agent 归一化。Runner 通过统一的 `Transport`
...
| 维度 | Agent Cloud | Codex CLI | Claude Code | Gemini CLI | OpenCode |
| --- | --- | --- | --- | --- | --- |
...
远程恢复、凭据委派和跨 Agent 可观测性通常交由宿主环境。Agent Cloud 为
显式拥有这些责任而承担分布式系统复杂度。
...
对 Agent Cloud 的启示是保留 Codex 原生项的原始记录，并仅把稳定的公共事件
...
Agent Cloud 应将“会话状态”和“执行策略”拆开：同一 Prompt 在不同组织、
...
Agent Cloud 可采用其项目上下文思想，但不能沿用本地存储假设。持久对象应为
...
Agent Cloud 应借鉴该服务边界，但保留控制面/Runner 数据面的分离。公开对话
