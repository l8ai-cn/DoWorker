...
const BUILT_IN_TYPES = new Set(Object.values(LOOM_BLOCK_TYPES));
...
function readInstructionChain(
  ...
  for (let block = first; block; block = block.getNextBlock()) {
    if (block.type === LOOM_BLOCK_TYPES.instruction) {
...
function readAcceptanceChain(
  ...
  for (let block = first; block; block = block.getNextBlock()) {
    if (block.type === LOOM_BLOCK_TYPES.acceptance) {
...
export function workspaceToDraft(
  ...
  const root = workspace.getBlocksByType(LOOM_BLOCK_TYPES.root, false)[0];
