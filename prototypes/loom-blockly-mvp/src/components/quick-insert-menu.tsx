...
import { LOOM_BLOCK_TYPES } from "../blockly/block-catalog";
import {
  customBlockType,
...
const ITEMS = [
  ["Goal Loop", LOOM_BLOCK_TYPES.root],
  ["使用 Worker", LOOM_BLOCK_TYPES.worker],
  ["执行任务", LOOM_BLOCK_TYPES.instruction],
  ["验收条件", LOOM_BLOCK_TYPES.acceptance],
  ["运行验证命令", LOOM_BLOCK_TYPES.verifier],
  ["执行边界", LOOM_BLOCK_TYPES.limits],
  ["失败处理", LOOM_BLOCK_TYPES.escalation],
] as const;
  ...
  return (
    ...
    >
      ...
      {ITEMS.map(([label, type]) => (
        <button
          disabled={type === LOOM_BLOCK_TYPES.root && hasRoot}
          key={type}
