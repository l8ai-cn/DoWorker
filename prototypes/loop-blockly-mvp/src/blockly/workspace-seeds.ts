...
export function createGoalRoot(
  ...
  return insertLoopBlock(workspace, LOOP_BLOCK_TYPES.root, 72, 52);
}
...
export function loadExampleProgram(
  ...
  workspace.clear();
  const root = block(workspace, LOOP_BLOCK_TYPES.root);
  const worker = block(workspace, LOOP_BLOCK_TYPES.worker);
  const task = block(workspace, LOOP_BLOCK_TYPES.instruction);
  const acceptance = block(workspace, LOOP_BLOCK_TYPES.acceptance);
  const verifier = block(workspace, LOOP_BLOCK_TYPES.verifier);
  const limits = block(workspace, LOOP_BLOCK_TYPES.limits);
  const escalation = block(workspace, LOOP_BLOCK_TYPES.escalation);
