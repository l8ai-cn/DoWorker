...
async function main() {
  ...
  try {
    ...
    const session = await createSession(
      token,
      sessionIDs,
      { agent_id: "e2e-echo", title: "S5 parity" },
    );
    ...
    if (delRes.ok) untrackSmokeSession(sessionIDs, sid);
  } catch (error) {
    step("S5 smoke run", false, String(error));
  } finally {
    if (token) {
      try {
        step("S5 fixture cleanup", true, `deleted=${await cleanupSmokeSessions(token, sessionIDs)}`);
      } catch (error) {
        step("S5 fixture cleanup", false, String(error));
      }
    }
  }
